...
| Todo                     | Use the explicit idea entrance or an optional validation follow-up; describe the request naturally.                               | Success closes the composer and shows a brief notice. The local list keeps summary metadata only; complete content belongs on GitHub. No extra Done step or local close/reopen control. |
...
Issue, closes the composer, and shows a brief non-modal notice. There is no
success-result dialog or Done step. Users do not enter Issue title, deferral
...
for an Action without output. The prepared creation payload contains that context,
but the local Todo record keeps only a title, bounded summary, number, labels,
status, optional URL, and Action association, not the full request or origin
context. Recording an Issue does not resolve review blockers.

The Todo UI is a compact issue list with no local context expansion or
close/reopen buttons. Real Issue URLs will open GitHub; fictional examples show
GitHub as not connected instead of inventing links. Promoting a Todo into a task
under a selected Node remains a later product direction, not a new demo control.
