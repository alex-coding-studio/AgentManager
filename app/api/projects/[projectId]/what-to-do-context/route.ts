...
export async function GET(
  ...
  try {
    return Response.json(
      { instructions: await readWhatToDoInstructions(project) },
...
export async function PATCH(
  ...
  try {
    ...
    return Response.json(await saveWhatToDoInstructions(project, instructions));
  } catch (error) {
    return failure(error);
  }
}
...
function failure(error: unknown) {
  return apiErrorResponse(
    ...
    '/api/projects/[projectId]/what-to-do-context',
  );
}
